...
var sampleStartImage = [
    [
        ...
        [191, 342, 148, 320, 1],
        [148, 286, 141, 266, 1],
        [153, 252, 167.5, 235.5, 0],
        [182, 219, 167.5, 235.5, 0]
    ],
    [
        [379, 62, 425, 106, 1],
        [390, 177, 485, 147, 1],
        ...
        [486, 464, 470, 486, 1],
        [423, 489, 375, 458.5, 1],
        [327, 428, 288.2828729281768, 449.9745856353591, 0],
        [253, 470, 189, 506, 1],
        [150, 453, 129, 398, 1],
        [206, 387, 253, 340, 1],
        [241, 276, 200.5, 260, 0],
...
function makerbotAccountClick(a) {
    if (itemClosed && allPoints.length > 0) {
        uploadToMakerBotAccount(a)
    } else {
        alert("As soon as you complete your cookie cutter, you'll be able to upload it to your Makerbot account.")
    }
}

function makerbotAccountClickWithTitle(a) {
    var b;
    if (itemClosed && allPoints.length > 0) {
        while (true) {
            b = prompt("Choose a title for your cookie cutter:");
            break
        }
        if (b) {
            uploadToMakerBotAccount(a, "&title=" + b)
        }
    } else {
        alert("As soon as you complete your cookie cutter, you'll be able to upload it to your Makerbot account.")
    }
...
function uploadToMakerBotAccount(b, a) {
    if (!itemClosed) {
        alert("Please close the points on your cookie cutter")
    } else {
        if (window.waiting_for_makerbot_upload) {} else {
            form = $("#downloadForm");
            form.find('input[name="points"]').val(JSON.stringify(allPoints));
            form.find('input[name="type"]').val("stl");
            form.find('input[name="size"]').val($("#cookiecuttersizedropdown").val());
            var c = parseFloat($("#cookiecutterheightdropdown").val()) * 25.4;
            form.find('input[name="height"]').val(c);
            form.find('input[name="thickness"]').val($("#cookiecutterthicknessdropdown").val());
            $("#loading-to-makerbot-thinking").fadeIn();
            window.waiting_for_makerbot_upload = true;
            var d = $("#downloadForm").serialize();
            if (a) {
                d += a
            }
            $.post(b, d).done(function() {
                alert("Uploaded! Your cookie cutter should be available in your MakerBot account in less than 5 minutes.")
            }).fail(function() {
                alert("Sorry, we had trouble uploading that to your MakerBot account")
            }).always(function() {
                window.waiting_for_makerbot_upload = false;
                $("#loading-to-makerbot-thinking").fadeOut()
            })
        }
    }
}
    ...
    if (itemClosed) {
        ...
        $.ajax({
            ...
            error: function(e, g, d) {
                $("#alerts").empty().append("Sorry, we've had an error creating that 3D model. This almost always means that some lines were drawn too close together (or intersecting), which will lead to a bad cookie cutter - try again, ensuring there are no thin sections. You can also try picking a larger cookie cutter 'max size' below.").css("color", "red").show();
